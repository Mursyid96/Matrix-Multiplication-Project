        ...
        for (int j = 0; j < squaredP; j++)
        {
            ...
            if (my_rank == col_shifted)
            {
                if (my_rank != col_original)
                {
                    ...
                    MPI_Isend(&(localB[0][0]), row2_local * column2_local, MPI_INT, col_original, 0, MPI_COMM_WORLD,&request);
                    MPI_Wait(&request,&status);
                    sent = true;
                }
                else
            ...
            {
                    // printf("sender: %d\n",col_shifted);
                    MPI_Irecv(&(row_receive[0][0]), row2_local * column2_local, MPI_INT, col_shifted, 0, MPI_COMM_WORLD, &request);
                    MPI_Wait(&request,&status);
                    receive = true; 
            }
